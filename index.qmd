...
checkboxGroupInput(
  ...
  choices = c("Uncheck All Boxes", players1),
  inline = TRUE
)
...
checkboxGroupInput(
  ...
  choices = c("Uncheck All Boxes", players2),
  inline = TRUE
)
...
observeEvent(input$blockers1, {
  if ("Uncheck All Boxes" %in% input$blockers1) {
...
observeEvent(input$blockers2, {
  if ("Uncheck All Boxes" %in% input$blockers2) {
    updateCheckboxGroupInput(
      session, 
      "blockers2", 
      selected = character(0)
...
tidy_individual1 <- reactive({
  data$df |> 
    ...
    reframe(
      ...
      n_jams_lead = sum(lead == team1),
      n_points_list = list(points1),
      n_points_sum = sum(points1)
    ) |> 
    mutate(
      team = team1,
      n_points_jam = round(n_points_sum / n_jams, 0)
    ) |> 
    rename(
      jammer = jammer1
    )
...
tidy_individual2 <- reactive({
  data$df |> 
    ...
    reframe(
      n_jams = n(),
      which_jams = list(jam_number),
      n_jams_lead = sum(lead == team2),
      n_points_list = list(points2),
      n_points_sum = sum(points2)
    ) |> 
    mutate(
      team = team2,
      n_points_jam = round(n_points_sum / n_jams, 0)
